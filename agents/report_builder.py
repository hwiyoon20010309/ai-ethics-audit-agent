...
def generate_report(service_info: dict,
    ...
    for k, v in initial_assessment.items():
        ...
        comment = v.get("comment", "") if isinstance(v, dict) else str(v)
        elems.append(Paragraph(f"<b>{k}</b>: {score}점<br/>{_format_text_for_pdf(comment)}", styles["NormalKor"]))
    ...
    # --- 피드백 반영 후 재평가 결과 ---
    elems.append(Paragraph("Ⅳ. 피드백 반영 후 재평가 결과", styles["Heading1Kor"]))
    elems.append(Spacer(1, 4))

    for k, v in final_assessment.items():
        # 🚫 Summary 항목은 점수 없이 텍스트만 표시
        if k.lower() == "summary":
            elems.append(Paragraph(f"<b>{k}</b>", styles["NormalKor"]))
            elems.append(Paragraph(_format_text_for_pdf(v.get("comment", "")), styles["NormalKor"]))
            elems.append(Spacer(1, 6))
            continue  # 🔥 여기서 꼭 continue 추가해야 함!!

        # ✅ 일반 항목은 점수와 코멘트 표시
        score = v.get("score", "-") if isinstance(v, dict) else "-"
        comment = v.get("comment", "") if isinstance(v, dict) else str(v)
        elems.append(
            Paragraph(f"<b>{k}</b>: {score}점<br/>{_format_text_for_pdf(comment)}", styles["NormalKor"])
        )
        elems.append(Spacer(1, 4))
